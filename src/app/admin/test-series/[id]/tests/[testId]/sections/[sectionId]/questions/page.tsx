...
import { useSectionQuestions, Question, QuestionType } from "@/hooks/test-series";
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-4 flex-1">
          ...
          <div className="flex-1 min-w-0 space-y-3">
            <div>
              <h4 className="text-base font-semibold leading-relaxed text-foreground mb-2">{question.text}</h4>
              {question.imageUrl && (
                <img
            ...
            <div className="flex items-center flex-wrap gap-2">
              <Badge variant="outline" className="text-xs font-medium bg-background border-border">
                {question.type}
              </Badge>
              <Badge variant="outline" className="text-xs font-medium bg-background border-border">
              ...
              {question.negativeMarks > 0 && (
                <Badge variant="outline" className="text-xs font-medium text-destructive bg-destructive/10 border-destructive/20">
...
function VirtualizedQuestions({ items }: { items: Question[] }) {
  ...
  return (
    <div ref={containerRef} className="flex-1 overflow-y-auto pr-2" style={{ willChange: "transform" }}>
...
export default function SectionQuestionsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return questions.filter((q) => {
      ...
      const matchesType = typeFilter === "ALL" ? true : q.type === (typeFilter as QuestionType);
      ...
      <div className="sticky top-0 z-10 border-b bg-background">
        <div className="flex items-center justify-between gap-3 px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-xs text-muted-foreground leading-none mb-0.5">Section</div>
              <div className="text-base font-semibold leading-tight">Questions</div>
            </div>
          </div>
